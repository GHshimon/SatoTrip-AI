...
import { getBulkAddJobStatus, bulkAddSpotsByPrefecture, BulkAddResponse, BulkAddJobStatus } from '../src/api/spots';
...
export const AdminSpots: React.FC = () => {
  ...
  let filteredSpots = spots.filter(s =>
    ...
    (!filterMissingLocation || !s.location) &&
    (!filterMissingImage || !s.image)
  );
  ...
  const areas = ['All', ...Array.from(new Set(spots.map(s => s.area))).sort()];
  const categories = ['All', ...Array.from(new Set(spots.map(s => s.category))).sort()];
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div className="p-4 border-b border-gray-100 flex flex-col md:flex-row gap-4 items-center justify-between bg-gray-50/50">
          ...
          <div className="flex flex-wrap gap-2 w-full md:w-auto items-center">
            <select className="px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white" value={filterArea} onChange={e => { setFilterArea(e.target.value); setCurrentPage(1); }}>
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                ...
                >
                  ...
                  {['Food', 'Nature', 'History', 'Art', 'Shopping', 'Relax', 'Culture'].map(c => (
                    <option key={c} value={c}>{c}</option>
                  ))}
                </select>
                <p className="text-xs text-text-muted mt-1">
                  特定のカテゴリに絞って検索します（例: Food -&gt; グルメ, ランチ...）
                </p>
              </div>
            ...
            <div className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                  ...
                  >
                    {['History', 'Nature', 'Food', 'Culture', 'Shopping', 'Art', 'Relax'].map(c => (
                      <option key={c} value={c}>{c}</option>
                    ))}
                  </select>
                </div>
