...
def get_spots_for_plan(
    ...
    if tags and result:
        ...
        for spot in result:
            ...
            # タグがリスト形式の場合
            if isinstance(spot_tags, list):
                # いずれかのタグがマッチするかチェック
                if any(tag in spot_tags or any(tag in str(t) for t in spot_tags) for tag in tags):
                    filtered_result.append(spot)
            # タグが文字列形式の場合
            elif isinstance(spot_tags, str):
                if any(tag in spot_tags for tag in tags):
                    filtered_result.append(spot)
            # タグがない場合はスキップ
